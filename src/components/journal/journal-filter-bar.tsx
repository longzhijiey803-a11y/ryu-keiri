...
import { DEPARTMENTS, PROJECTS } from "@/lib/journal-data";
import {
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border border-border bg-surface p-3">
      ...
      <Select
        value={filter.status}
        onValueChange={(v) =>
          set({ status: v as JournalFilter["status"] })
        }
      >
      ...
      >
        ...
        <SelectContent>
          <SelectItem value="all">すべてのPJ</SelectItem>
          {PROJECTS.map((p) => (
